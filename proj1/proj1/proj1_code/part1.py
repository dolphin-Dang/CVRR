...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    # Gaussion1D = const * exp(coefficient * ((x-mu)**2))
    mu = floor(ksize/2)
    const = 1 / (np.sqrt(2*np.pi)*sigma)
    coefficient = -1 / (2*(sigma**2))
    for x in range(ksize):
      value = const * np.exp(coefficient * ((x-mu)**2))
      kernel[x] = value
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    kernel1D = create_Gaussian_kernel_1D(k, cutoff_frequency)
    kernel = np.outer(kernel1D, kernel1D)
    
    """
    Note: outer product keeps the sum=1.
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    k_pad = k//2
    j_pad = j//2
    image_padded = np.pad(image, ((k_pad, k_pad),(j_pad, j_pad),(0,0)), 'constant')
    ...
    for channel in range(c):
      for mi in range(m):
        for ni in range(n):
          # cross-correlation
          image_piece = image_padded[mi:mi+k, ni:ni+j, channel]
          filtered_image[mi,ni,channel] = np.sum(np.multiply(image_piece, filter))
