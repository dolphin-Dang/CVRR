...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernel = np.tile(kernel.reshape(1,1,*kernel.shape), (self.n_channels,1,1,1))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        ### TODO: YOUR CODE HERE ###
        k = kernel.shape[-1]
        filtered_image = F.conv2d(x, kernel, padding=k//2, groups=self.n_channels)
